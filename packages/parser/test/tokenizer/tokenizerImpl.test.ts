...
import { SPEC, TokenTypes } from "../../src/constants/dadLangSpec";
import InvalidStateException from "../../src/exceptions/invalidStateException";

const tokenizer = new TokenizerImpl(SPEC);

test("test Tokenizer.getNextToken with PROGRAM_START should success", () => {
  tokenizer.initTokenizer(`idhar aao`);

  expect(tokenizer.getNextToken()).toStrictEqual({
    type: TokenTypes.PROGRAM_START,
    value: "idhar aao",
  });
});

test("test Tokenizer.getNextToken with PROGRAM_END should success", () => {
  tokenizer.initTokenizer(`jao padhai karo`);

  expect(tokenizer.getNextToken()).toStrictEqual({
    type: TokenTypes.PROGRAM_END,
    value: "jao padhai karo",
  });
});
