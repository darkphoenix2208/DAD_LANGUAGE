...
export default class DadLangModule {
  ...
  private static _sleepStatement?: SleepStatement;
  private static _tryCatchStatement?: TryCatchStatement;
  private static _arrayExpression?: ArrayExpression;
  private static _memberExpression?: MemberExpression;
  private static _classDeclaration?: ClassDeclaration;
  private static _switchStatement?: SwitchStatement;
  private static _newExpression?: NewExpression;
  private static _thisExpression?: ThisExpression;
  ...
  static getSleepStatement() {
    if (!this._sleepStatement) {
      this._sleepStatement = new SleepStatement(this.getTokenExecutor());
    }

    return this._sleepStatement;
  }

  static getTryCatchStatement() {
    if (!this._tryCatchStatement) {
      this._tryCatchStatement = new TryCatchStatement(
        this.getTokenExecutor(),
        this.getStatementList()
      );
    }
  ...
  static getArrayExpression() {
    if (!this._arrayExpression) {
      this._arrayExpression = new ArrayExpression(this.getTokenExecutor());
    }
    return this._arrayExpression;
  }

  static getMemberExpression() {
    if (!this._memberExpression) {
      this._memberExpression = new MemberExpression(this.getTokenExecutor());
    }
    return this._memberExpression;
  }

  static getClassDeclaration() {
    if (!this._classDeclaration) {
      this._classDeclaration = new ClassDeclaration(this.getTokenExecutor());
    }
    return this._classDeclaration;
  }

  static getSwitchStatement() {
    if (!this._switchStatement) {
      this._switchStatement = new SwitchStatement(this.getTokenExecutor());
    }
    return this._switchStatement;
  }

  static getNewExpression() {
    if (!this._newExpression) {
      this._newExpression = new NewExpression(this.getTokenExecutor());
    }
    return this._newExpression;
  }

  static getThisExpression() {
    if (!this._thisExpression) {
      this._thisExpression = new ThisExpression(this.getTokenExecutor());
    }
